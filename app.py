...
def _proxy_inbounds(listen):
    return [
        {'port': 1080, 'listen': listen, 'protocol': 'socks', 'settings': {'udp': True}},
...
def xray_diagnose():
    ...
    has_socks = any(ib.get('port') == 1080 and ib.get('protocol') == 'socks' for ib in inbounds)
    return {
    ...
    for i, ob in enumerate(cfg.get('outbounds', [])):
        ...
        if not tag:
            if proto == 'vless':
                ...
                tag = 'api' if any(
                    ib.get('tag') == 'api' for ib in cfg.get('inbounds', [])
                ) and 'api' not in used_tags and 'direct' in used_tags else 'direct'
            else:
                tag = f'outbound-{i}'
            ob['tag'] = tag
        base = ob['tag']
...
def parse_vless(link):
    ...
    if not uid or not server or port is None:
        return None
    result = {'uid': uid, 'server': server, 'host': server, 'port': int(port), 'link': link}
    ...
    country_m = re.search(r'^([A-Z]{2})\b', fragment) or re.search(r'[#\s]([A-Z]{2})\b', fragment)
...
def enrich_all_unknown_countries():
    ...
    rows = db_q("SELECT id, host FROM proxies WHERE country IS NULL OR country = '' OR length(country) > 2")
    for r in rows:
...
def _reality_server_name(parsed):
    return _vless_param(parsed, 'sni', 'serverName') or parsed.get('headerHost') or parsed['server']
...
def stream_settings(parsed):
    ...
    if sec == 'tls':
        ...
        s['security'] = 'reality'
        r = {'serverName': _reality_server_name(parsed), 'fingerprint': _vless_param(parsed, 'fp', 'fingerprint') or 'chrome',
             'show': False, 'publicKey': _vless_param(parsed, 'pbk', 'publicKey', 'publickey'),
...
def generate_full_config():
    ...
    rows = db_q("SELECT link FROM proxies WHERE status='working' AND country != '' AND country != 'RU'")
    proxy_obs = []
    for i, r in enumerate(rows):
        parsed = parse_vless(r['link'])
        if parsed:
            proxy_obs.append(_build_outbound(parsed, f'node{i}'))
    outbounds = proxy_obs + [{'protocol': 'freedom', 'tag': 'direct'},
...
def apply_all_proxies():
    """Generate config with all working world proxies and apply."""
    config = generate_full_config()
    raw = json.dumps(config, indent=2)
    cfg_path = xray_config_path()
    cfg_path.parent.mkdir(parents=True, exist_ok=True)
    cfg_path.write_text(raw)
    proxy_count = sum(1 for o in config["outbounds"] if o["tag"].startswith("node"))
    add_log('INFO', f'Config written with {proxy_count} proxies')

    if xray_api_ok():
        for tag in list_active_outbounds():
            if tag.startswith('node'):
                try:
                    subprocess.run([xray_bin(), 'api', 'removeoutbound', '-s', '127.0.0.1:10085', '--tag', tag],
                                   capture_output=True, timeout=10)
                except Exception:
                    pass
        for ob in config['outbounds']:
            if not ob['tag'].startswith('node'):
                continue
            ...
            with tmp:
                ...
                tmp_path = tmp.name
            try:
                subprocess.run([xray_bin(), 'api', 'addoutbound', '-s', '127.0.0.1:10085', tmp_path],
                               capture_output=True, timeout=10)
            except Exception:
                pass
            finally:
...
def api_status():
    ...
    failed_recent = db_q("SELECT COUNT(*) c FROM proxies WHERE status='failed' AND (failed_since IS NULL OR failed_since >= datetime('now', '-24 hours'))")[0]['c']
    failed_old = db_q("SELECT COUNT(*) c FROM proxies WHERE status='failed' AND failed_since < datetime('now', '-24 hours')")[0]['c']
    ru = db_q("SELECT COUNT(*) c FROM proxies WHERE status='working' AND country='RU'")[0]['c']
    world = db_q("SELECT COUNT(*) c FROM proxies WHERE status='working' AND country != '' AND country != 'RU'")[0]['c']
    return jsonify(total=total, working=working, failed_recent=failed_recent, failed_old=failed_old,
                   ru=ru, world=world)
...
def api_sources_add():
    ...
    try:
        db_q("INSERT INTO sources (name, url, created_at) VALUES (?, ?, ?)",
...
def api_xray_outbounds():
    ...
    traffic = {}
    try:
        r = subprocess.run([xray_bin(), 'api', 'statsquery', '-s', '127.0.0.1:10085'],
                           capture_output=True, text=True, timeout=5)
        for line in r.stdout.splitlines():
            m = re.search(r'outbound>>>([^>]+)>>>traffic>>>([a-z]+)', line)
            if m:
