...
*   **📊 Speed test** — После VLESS-теста топ-20 самых быстрых по пингу замеряют реальную пропускную способность (скачивание 100KB файла через прокси). Результат — `speed_kbps` — сохраняется в БД и используется как primary сортировка при сборке конфига.
...
| Speed test enabled | true | Вкл/выкл замер скорости после VLESS-теста |
| Speed test max | 20 | Сколько самых быстрых прокси тестировать |
| Speed test URL | `http://proof.ovh.net/files/100Kb.dat` | Файл для скачивания (HTTP, не Cloudflare)
